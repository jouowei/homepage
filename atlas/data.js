...
/* ───────────── 信用側每條線的機制說明（為什麼信用是這樣流動的） ─────────────
   鍵為「上游>下游」。來源：DOC-9 §7.1–7.3、§8.1、§9.2、§10.1、§11.1/§11.7、§12、§14；DOC-3 融資註 2026-07-02。 */
window.ATLAS.creditWhy = {
  /* 來源 → 管道 */
  's_fed>p_ig': '財政部賣短券、Fed 以新造準備金買短券、財政部再買回 10-30 年舊債：三段各自不算印鈔，接通後長端存量被收走、新準備金進入系統。購債窗 2026-09-09 至 11-04 期間長端是受管理價格，投資級發債就是對著這個被托住的長端定價；接錯節點②（單機構口徑只見一段）就在這條線上。',
  's_fed>p_bank': '新準備金先落在銀行資產負債表，理論上是工商放款的彈藥；但財政部短券的吸收方已無隔夜逆回購緩衝，直接從銀行準備金抽水。準備金地板的三個閾值是在「單一抽水管」假設下設的，這條線現在有兩根管子在抽。',
  's_bank>p_ig': '投資級與否決定的不是報酬率高低，而是保險與退休金資本能不能在資本費用規則下持有它。受監管資本的持有資格是真正的放行閥，所以大型雲端商把發債集中在投資級公開市場；接錯節點③（誰有資格放這筆款）在這裡。',
  's_bank>p_pc': '銀行資本規則放不下的那一段，流到規則外的私募信貸：BIS 年報記錄私募信貸的 AI 放款餘額由近零升至逾 $200B，占比由不到 1% 升到約 8%。這條線是分子四支儀表全部量不到的邊際管道。',
  's_bank>p_vendor': '承租人沒有評等時，市場不放棄，而是製造一個替代品：以特殊目的機構加供應商殘值擔保，把一筆本應落在高收益區間的債務轉成投資級紙張，讓受監管資本能持有。整座結構的存在理由就是繞過這條規則（§14.3）。',
  's_asia>p_ig': '亞洲資金地板上抬（BOJ 1.0%、BOK 2.75%）加上台灣金管會逼壽險降美元曝險，把美國投資級債的邊際外國買盤往回抽。雲端商轉向歐元、日圓、瑞郎發債正是迴避美元市場飽和；接錯節點④（四支儀表全量美國信用）在這條線上。',
  's_broad>p_ig': '廣義高收益與投資級利差位於歷史區間低端，判別器落在右上格：要求報酬率紀律不構成約束。對稱鎖同時成立：資本被強迫部署時，最差的專案也拿得到融資。',
  's_broad>p_pc': '同一格的另一面：利差壓在低端時，私募信貸基金與保險資本在 4(a)(2) 私募市場直接進場，單筆規模開始要求 $2-3B。廣義儀表對這裡 SILENT，讀起來正常。',
  's_broad>p_abs': '寬鬆的廣義信用環境讓資料中心租約可以被證券化：ABS 加 CMBS 存量由 $4B 升到 $61B。租約擔保票據九倍超額認購，說明的是資本的飢餓，不是專案的品質。',
  /* 管道 → 承載者 */
  'p_ig>n_hyper': '大型雲端商有投資級評等與營運現金流，但 2026 資本支出約等於 100% 營運現金流（十年均值 40%），2028 預計 capex 1.29 兆對 FCF 2,300 億，差額只能靠借。三家 2026 迄今發債近 $220B，投資級公開市場是它們的主管道。',
  'p_pc>n_lab': 'AI 實驗室與 neocloud 沒有評等、沒有公開利差，銀行與公開債市進不去，私募信貸是它們建置資金的邊際來源。真正的閥門在這裡：大型租戶的信用集中度，而系統目前無任何接線（§7.2）。',
  'p_pc>n_hyper': '雲端商也用私募市場消化公開市場吃不下的量：百年歷史的 4(a)(2) 私募市場成為 AI 融資新戰場，保險資本直接對接。',
  'p_abs>n_lab': '以 build-to-suit 資料中心的 15 年三重淨租為底層資產證券化。久期錯配、設備壽命、產出通縮四項風險依合約全部留在租戶的資產負債表上，證券化只是把租金流賣出去。',
  'p_abs>n_hyper': '雲端商的租約是證券化最好的擔保：投資級租戶的長約讓 DC ABS 可以拿到低票息。這條線把雲端商的信用借給了開發商。',
  'p_bank>n_lab': '銀行與公開債市的角色大於原先預期的私募主導。IREN 的 $3.65B GPU 融資包能拿到 Fitch A、混合成本 6.00%，靠的是 MSFT 五年合約當擔保：租戶信用經由合約傳導成融資條件。',
  'p_bank>n_hyper': '銀行對雲端商是承銷與工商放款雙重受益方；GS 共同主辦德州 5GW 私有燃氣發電專案，股權與信貸並募，代表資本市場已為電力瓶頸定價。',
  'p_vendor>n_lab': '承租人沒有信用讀數時，風險被搬到願意出借評等的那一方帳上：供應商以自身投資級信用書寫殘值支持（NVIDIA 於 PORTS-Pike 出資入股並提供上限 $105B 擔保，同時是排他算力供應商）。收費站收租、承擔為零；擔保人不收租、承擔尾部。可讀的節點不在承租人，在供應商的或有負債揭露。',
  /* 承載者 → 信用流 */
  'n_hyper>f_ai': '雲端商的資本支出就是 AI 基建這股信用流的本體：2026 四家合計約 $725B，較 2025 增加 77%。融資天花板目前領先物理天花板一個身位。',
  'n_lab>f_ai': '實驗室的算力承諾（例如以 GW 計的多年合約）透過 neocloud 與雲端商變成建置支出。合約 GW 公開，實際通電的 GW 不公開，兩者不是同一個數；這段是六段傳導鏈裡看不見的前三格。',
  'n_sov>f_sov': '主權 AI 基建由政府補貼與主權基金直投出資，政治上不可撤回，不受 ROI 殺開關控制。它和 AI 基建撞同一組 EUV、先進製程與電力，但先被擠出的是企業客戶。'
};
